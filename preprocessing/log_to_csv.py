import pandas as pd
class LogtoCsv:
    def __init__(self, filename_cmx, dir, fileout, filename_pwr, offsettimepwr,hdegree):
        self.fileid_cmx = filename_cmx
        self.dir = dir
        self.fileout = fileout
        self.fileid_pwr = filename_pwr
        self.fs = 5000
        self.meanreso = 200 # was 4000
        self.offsettimepwr = offsettimepwr*self.fs #0.883*self.fs # nr of lines to be removed
        self.hdegree = hdegree # first col degree value
    ...
    def LogtoCsvDef(self):
        ...
        #write file
        f = self.writefile()
        f.write('H_degree,DL_LteThp(bps),DL_NrDlThp(bps),DL_LteAck%,DL_LteNack%,DL_LteDtx%,DL_LteBLER%,DL_LteThp%,DL_NrAck%,DL_NrNack%,DL_NrDtx%,DL_NrBLER%,DL_NrThp%,UL_LteAck%,UL_LteNack%,UL_LteDtx%,UL_LteBLER%,UL_NrAck%,UL_NrNack%,UL_NrDtx%,UL_NrBLER%,UL_LteThp%,UL_LteTph(bps),UL_NrThp%,UL_NrTph(bps),Lte_RSRP(dBm),Lte_RSRQ(dBm),NR_RSRP(dBm),NR_RSRQ(dBm),NR_SINR(dBm),UE_PowerMeas(mW),NRULpwr(dBm),NRDLpwr(dBm),LTEULpwr(dBm),LTEDLpwr(dBm) \n')
        ...
        while (lineIdx+loglength) < linelen:
            ...
            if readlines[lineIdx].find('%') != -1 and (lineIdx+loglength) > linelen:
                ...
                if self.hdegree == 0:
                    f.write(timestamp + ', ') # time stamp
                else:
                    f.write(str(self.hdegree) + ', ')
            ...
            elif readlines[lineIdx].find('CONFigure:SIGNaling:NRADio:CELL:POWer:DL:TCELl?') != -1 and lineIdx > 9: #NR DL power
                lineIdx += 2
                readData = readlines[lineIdx].replace('\'', '').replace('[', '').replace(']', '').replace('\n', '').split(",")
                #print(readData, ',lineid=', lineIdx)
                NRDLpwr = int(self.checknav(readData[0]))
