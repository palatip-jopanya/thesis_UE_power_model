...
def csvTocorr(fileID):
   ...
   # {‘pearson’, ‘kendall’, ‘spearman’} or callable
    ...
    #type = "Spearman"
    type = "Pearson"
    corr_matrix = dataFrame.corr( method=type.lower())
    corr_matrix = dataFrame.corr(method=type)
    ...
    powermeas_corr = dataFrame.corr()[['UE_PowerMeas(mW)']].sort_values(by='UE_PowerMeas(mW)', ascending=False)
...
def callfunc(mode):
    ...
    if mode == 's':
        ...
        for file in os.listdir(path_path):
            if file.find('csv') != -1 and file.find('rmvCol') != -1:
                fullpath = path_path + '/' + file
                csvTocorr(fullpath)
